                          ...
                          Y = NULL)
{
  ...
  if (pracma::strcmpi(method, "Laplace")){
    ...
    InfoApprox <- laplaceApproximation(opts$nY, X, b_mean, b_var, nChoiceSet)
    
  } else if (pracma::strcmpi(method, "MQL")){
    
    InfoApprox <- MQLApprox(X, b_mean, b_var, nChoiceSet)
    
  } else if (pracma::strcmpi(method, "PQL")){
    
    InfoApprox <- PQLApprox(opts$nU, X, b_mean, b_var, nChoiceSet)
    ...
    f <- InfoApproxTemp[1:(nrow(InfoApproxTemp) / 2), ]
    d <- InfoApproxTemp[(nrow(InfoApproxTemp) / 2 +1):nrow(InfoApproxTemp),]
    InfoApprox = t(f) %*% MASS::ginv(d) %*% f
  ...
  InfoApproxFinal <- matrix(0, nrow=length(b_mean) + nRandEffect, ncol=length(b_mean) + nRandEffect)

  InfoApproxFinal[c(randomEffectIndices, fixedEffectIndices, (length(b_mean)+1):(length(b_mean) + nRandEffect) ),
                  c(randomEffectIndices, fixedEffectIndices, (length(b_mean)+1):(length(b_mean) + nRandEffect) )] <- 
    InfoApprox[ c(1:length(b_mean), 1:nRandEffect + length(b_mean)), c(1:length(b_mean), 1:nRandEffect + length(b_mean))]
  
  
  return(InfoApproxFinal)
